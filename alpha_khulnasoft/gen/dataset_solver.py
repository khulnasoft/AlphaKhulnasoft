...
from alpha_khulnasoft.gen.coding_competitor import CodeContestsCompetitor
from alpha_khulnasoft.gen.utils import evaluate_solution_on_subset
from alpha_khulnasoft.log import setup_logger, get_logger
        ...
        for iteration in range(setting.get("dataset.num_iterations", 1)):
            ...
            if not ((prev_iter == {}) or (prev_iter is None)):
                ...
                if is_solved(prev_iter):
                    logger.info(f"khulnasoft solved problem {problem_number} in iteration {iteration}")
                    break
                continue
            ...
            if is_solved(problem_database[problem_number][it_str]):
                ...
                logger.info(f"khulnasoft failed to solve problem {problem_number} in iteration {iteration}")
